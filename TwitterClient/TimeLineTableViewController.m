...
- (NSAttributedString *)labelAttributedString:(NSString *)labelString
{
    ...
    NSDictionary *atrributes = @{NSParagraphStyleAttributeName:paragraphStyle,
                                 NSFontAttributeName:font};
    
    NSMutableAttributedString *attributedText =
    [[NSMutableAttributedString alloc] initWithString:text attributes:atrributes];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DetailViewController *detailViewController =[self.storyboard instantiateViewControllerWithIdentifier:@"DetailViewController"];//ビューのidを使って次のビューのオブジェクトを作成
    detailViewController.text = cell.tweetTextLabel.text;//次のビューオブジェクトに値をセット
    ...
    [self.navigationController pushViewController:detailViewController animated:YES];//次のビューコントローラーに飛ぶセグエを使わずにナビゲーションコントローラーだから戻れる
